...
test.describe("12.4 — New /compare/*/for/* pages render for crawlers", () => {
  ...
  test("KNOWN GAP — the static /sitemap.xml on the main domain does not (yet) include niche pages", async ({
    request,
  }) => {
    // Two different sitemaps exist: a build-time static public/sitemap.xml
    // (generated by src/lib/generate-sitemap.ts) and a runtime dynamic
    // supabase/functions/sitemap edge function. Verified live 2026-07-14:
    // the static one at /sitemap.xml does NOT list /compare/*/for/* pages,
    // only the dynamic edge function does. If Google is configured to read
    // /sitemap.xml (the conventional location), the 36 new niche pages are
    // effectively undiscoverable via sitemap today even though the pages
    // themselves render correctly. Documents the finding rather than
    // silently passing.
    ...
    expect(
      xml,
      "static /sitemap.xml unexpectedly now includes niche pages — if this test starts failing, it likely means the gap was fixed; update this test to assert the positive case instead",
    ).not.toContain("/compare/servicetitan/for/");
  });
});
