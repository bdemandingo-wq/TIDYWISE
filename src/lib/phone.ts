 ...
 *
 * KEEP IN SYNC: `supabase/functions/_shared/phone.ts` is a verbatim copy below
 * its header, because the senders run in Deno and cannot import from `src/`.
 * phone.test.ts imports both and asserts they agree.
 *
 * WHY toE164 RETURNS NULL RATHER THAN A BEST EFFORT. The code this replaces
 * (notify-new-lead, step 12) only guarded the 10-digit case:
 *
 *     let toPhone = phone.replace(/\D/g, "");
 *     if (!toPhone.startsWith("1") && toPhone.length === 10) toPhone = "1" + toPhone;
 *     toPhone = "+" + toPhone;
 *
 * so a 9-digit number became "+405252227", an empty string became "+", and an
 * Australian mobile in national format became "+10409206947". All three were
 * handed to OpenPhone as a `to` address. A null the caller can log beats a
 ...
 *
 * Behaviourally identical to the private normalizePhone() that lived in
 * marketing-guard.ts before this module existed — phone.test.ts pins that
 * equivalence against a copy of the old body, so adopting it there is a
 * provable no-op rather than a hopeful one.
 *
