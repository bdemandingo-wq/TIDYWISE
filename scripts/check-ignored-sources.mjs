...
// Lovable-generated artifacts are not hand-written source; they are ignored by
// global convention (`*.lov.json`) and may be regenerated by the platform.
// Excluding them keeps the guard focused on real source files like components
// and pages, which is the failure mode it was written to catch.
const sourceFiles = ignored.filter((f) => !f.endsWith('.lov.json'));

if (sourceFiles.length) {
  console.error(
    `✖ ${sourceFiles.length} file${sourceFiles.length === 1 ? '' : 's'} under src/ ${
      sourceFiles.length === 1 ? 'is' : 'are'
    } git-ignored:\n`,
  );
...
if (ignored.length && !sourceFiles.length) {
  console.log('✓ no src/ source file is git-ignored (ignored Lovable artifacts excluded).');
  process.exit(0);
}
