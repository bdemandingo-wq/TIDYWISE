...
-- 1. New booking column
ALTER TABLE public.bookings
  ADD COLUMN IF NOT EXISTS has_pets boolean NOT NULL DEFAULT false;

-- 2. Extend organization_pricing_settings with pet + exclude parameters
ALTER TABLE public.organization_pricing_settings
  ADD COLUMN IF NOT EXISTS pet_fee numeric NOT NULL DEFAULT 25,
  ADD COLUMN IF NOT EXISTS pet_toggle_enabled boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS excluded_room_types text[] NOT NULL DEFAULT '{}'::text[],
  ADD COLUMN IF NOT EXISTS room_reduction_prices jsonb NOT NULL
    DEFAULT '{"bedroom":25,"bathroom":20,"full_bath":25}'::jsonb;

-- 3. Extend public booking settings RPC to also return pricing settings
