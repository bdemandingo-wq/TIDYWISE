...
import { refreshAccessToken } from "../gmail-refresh-token/index.ts";
...
export async function sendViaGmail(req: SendRequest): Promise<
...
> {
  ...
  if (!accessToken || expiresAt - Date.now() < 60_000) {
    const r = await refreshAccessToken(req.organization_id);
...
async function logFailure(supabase: any, req: SendRequest, error: string) {
  try {
